#!/usr/bin/env python3
"""Gaussian Process RNN hybrid model with LS=10 for epistemic uncertainty estimation.
...
T_MAX = 899
OUTNAME = os.environ.get('OUTNAME', "GP_RNN_LS10_Final_v1")
...
def fit_gp_rnn(rnn_gp_predictor, ae_model, max_samples=4000, n_inducing=1500):
    """Fit GP on RNN latent space features with LS10 kernel settings (matching AE GP)"""
    from utils import get_AE_loaders
    
    train_loaders, _ = get_AE_loaders(path="", data_distribution='get_KVS', batch_size=8, shuffle=True)
    
    train_feat_list, train_targ_list = [], []
    collected = 0
    
    rnn_gp_predictor.rnn_model.eval()
    ae_model.eval()
    ...
    with torch.no_grad():
        for data, target in train_loaders[0]:
            ...
            # Data from loader is (batch, channels, x, y, z) = 5D
            # But AE model expects 4D (t, h, d, w) when y='get_bottleneck'
            # So we need to squeeze the channel dimension: (batch, 1, x, y, z) -> (batch, x, y, z)
            data = torch.add(data[:, 0], 1.0).float().to(DEVICE)  # Remove channel dim: (batch, x, y, z)
            ...
            for b in range(batch_size):
                ...
                # Extract single sample: (x, y, z) = 3D, model expects 4D so add batch dim
                sample = data[b:b+1].to(DEVICE)  # (1, x, y, z) = 4D
                
                bottleneck = ae_model(sample, y='get_bottleneck')
                bottleneck_flat = bottleneck.view(bottleneck.shape[0], -1)
                
                bottleneck_seq = bottleneck_flat.unsqueeze(1).repeat(1, 25, 1).to(DEVICE)
                features = rnn_gp_predictor.rnn_model.extract_features(bottleneck_seq)
                
                # Handle target shape: target is (batch, channels, x, y, z)
                if len(target.shape) == 5:
                    tgt = target[b, 0, 12, 12, 12].cpu().numpy()
                elif len(target.shape) == 4:
                    tgt = target[b, 12, 12, 12].cpu().numpy()
                else:
                    tgt = target[b].cpu().numpy() if target.ndim > 0 else target.cpu().numpy()
                
                # Ensure tgt is at least 1D for concatenation
                if isinstance(tgt, np.ndarray) and tgt.ndim == 0:
                    tgt = np.array([tgt.item()])
                elif not isinstance(tgt, np.ndarray):
                    tgt = np.array([tgt])
                
                train_feat_list.append(features.cpu().numpy())
                train_targ_list.append(tgt)
                collected += features.shape[0]
    
    train_feat = np.vstack(train_feat_list) if train_feat_list else np.zeros((100, 256))
    train_targ = np.concatenate(train_targ_list) if train_targ_list else np.zeros((100,))
    
    n_inducing = min(n_inducing, train_feat.shape[0])
    idx = np.random.choice(train_feat.shape[0], size=n_inducing, replace=False)
    ...
    gp.fit(train_feat[idx], train_targ[idx])
    return gp

def create_gp_hybrid_ls10(ae_model, rnn_model_path):
    """Create GP hybrid model with LS10 kernel settings"""
    from model_rnn_gp import RNN_gp
    ...
    for rnn_gp in [rnn_gp_x, rnn_gp_y, rnn_gp_z]:
        rnn_old = RNN(256, 256, 25, 1, DEVICE)
        rnn_old.load_state_dict(state_dict)
        rnn_old.eval()
        rnn_gp.rnn_model.load_state_dict(rnn_old.state_dict())
        rnn_gp.rnn_model.eval()
        gp = fit_gp_rnn(rnn_gp, ae_model)
        rnn_gp.gp = gp
        rnn_gp.gp_trained = True
...
def process_inference(ae_model, rnn_model_path, file_name, t_max):
    ...
    hybrid_model = create_gp_hybrid_ls10(ae_model, rnn_model_path)
...
def main():
    ...
    for file_name in FILE_NAMES:
        mean_pred, epistemic = process_inference(ae_model, RNN_MODEL, file_name, T_MAX)
    ...
    predictions_files_tr = [process_from_tensor(ae_model, RNN_MODEL, datasets['truerandom'], file_idx * T_MAX, T_MAX) 
    ...
    predictions_files_ood = [process_from_tensor(ae_model, RNN_MODEL, datasets['ood'], file_idx * T_MAX, T_MAX) 
    ...
    predictions_files_cou = [process_from_tensor(ae_model, RNN_MODEL, datasets['couette'], file_idx * T_MAX, T_MAX) 
                             for file_idx in range(5)]
    ...
    save_method_results(
        method_name="GP_RNN_LS10", outname=OUTNAME, results_dict=results, epistemic_dict=epistemic, aleatoric_dict=aleatoric,
