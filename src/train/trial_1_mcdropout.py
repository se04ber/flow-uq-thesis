...
class CoverageWidthLoss(nn.Module):
    def __init__(self, alpha=0.95, lambd=0.5):
...
def trial_1_AE_aleatoric(alpha, alpha_string, train_loaders, valid_loaders):
    ...
    _model_identifier_i = f'McdropoutTest_50epochs_LR{alpha_string}_i_p01'
    ...
    timestamp = "template" #datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    np.save("train_losses_mc.npy",np.array(train_losses))
    np.save("valid_losses_mc.npy",np.array(val_losses))
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title('MC Dropout Loss Curves')
    plt.savefig('mc_dropout_loss_curves.png')
    plt.close()
    ...
    for data, target in valid_loaders[0]:
        ...
        mc_preds = mc_dropout_predict(_model_i, channel_data, mc_samples=mc_samples)
        mean_preds = mc_preds[:, :, 0, x, y, z]
        mean_of_means = np.mean(mean_preds, axis=0)
        epistemic_std = np.std(mean_preds, axis=0)
        log_var = mc_preds[:, :, 1, x, y, z]
        aleatoric_std = np.mean(np.sqrt(np.exp(log_var)), axis=0)
        all_mean_preds.append(mean_of_means)
        all_epistemic_stds.append(epistemic_std)
        all_aleatoric_stds.append(aleatoric_std)
        all_targets.append(channel_target[:, 0, x, y, z].cpu().numpy())
        all_mc_preds.append(mean_preds)
    mean_of_means = np.concatenate(all_mean_preds)
    epistemic_std = np.concatenate(all_epistemic_stds)
    aleatoric_std = np.concatenate(all_aleatoric_stds)
    target_vals = np.concatenate(all_targets)
    mc_preds = np.concatenate(all_mc_preds, axis=1)
    total_std = np.sqrt(epistemic_std**2 + aleatoric_std**2)
    time_steps = np.arange(len(target_vals))
    print("Target (first 5):", target_vals[:5])
    print("MC Dropout Mean Prediction (first 5):", mean_of_means[:5])
    print("Epistemic Std (first 5):", epistemic_std[:5])
    print("Aleatoric Std (first 5):", aleatoric_std[:5])
    print("MC Dropout Bias (mean diff):", np.mean(mean_of_means - target_vals))


    plt.figure(figsize=(12, 6))
    for i in range(mc_preds.shape[0]):
        plt.plot(time_steps, mc_preds[i], color='skyblue', linewidth=1, alpha=0.6, label='MC Sample' if i==0 else None)
    plt.plot(time_steps, target_vals, 'o-', color='black', linewidth=2, markersize=6, label='Target', alpha=0.8)
    plt.plot(time_steps, mean_of_means, 's-', color='orange', linewidth=2, markersize=6, label='MC Dropout Mean Prediction', alpha=0.8)
    plt.fill_between(time_steps, mean_of_means - epistemic_std, mean_of_means + epistemic_std, alpha=0.3, color='orange', label='±1σ Epistemic')
    plt.fill_between(time_steps, mean_of_means - aleatoric_std, mean_of_means + aleatoric_std, alpha=0.3, color='green', label='±1σ Aleatoric')
    plt.xlabel('Time Step')
    plt.ylabel('u_x Value')
    plt.title(f'MC Dropout AE: Target, Prediction, Aleatoric & Epistemic Uncertainty (Cell {x},{y},{z})')
    plt.legend(loc='upper right', fontsize=10)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'mc_dropout_aleatoric_epistemic_temporal_{timestamp}.png', dpi=150)
    plt.close()
    print(f"MC Dropout uncertainty plot saved to: mc_dropout_aleatoric_epistemic_temporal_{timestamp}.png")
    print(f"epistemic_std: shape={np.shape(epistemic_std)}, min={np.min(epistemic_std)}, max={np.max(epistemic_std)}, sample={epistemic_std[:5]}")
    ...
    print(f"mean_of_means: shape={np.shape(mean_of_means)}, min={np.min(mean_of_means)}, max={np.max(mean_of_means)}, sample={mean_of_means[:5]}")
    print(f"target_vals: shape={np.shape(target_vals)}, min={np.min(target_vals)}, max={np.max(target_vals)}, sample={target_vals[:5]}")
    #reference_target = np.load('npy_results/targets_reference.npy')
    np.save('mc_dropout_aleatoric_epistemic_lightweight_nll_results.npy', {
        'mean': mean_of_means,
        'std': aleatoric_std,
        'epistemic': epistemic_std
        #'target': reference_target
    })
    print('Saved NLL-based results for MC Dropout comparison.')
    return
...
def trial_1_AE_aleatoric_mp():
    ...
    _train_loaders,_valid_loaders = get_AE_loaders(path="",data_distribution="get_KVS",batch_size=8,shuffle=True)
