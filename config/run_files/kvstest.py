...
sys.path.append('/home/sabrina/MaMiCo/MaMiCo/build')
...
import os
kvsId = {os.environ.get('SLURM_ARRAY_TASK_ID')}
#print(type(kvsId))
kvsId = int(next(iter(kvsId)))

kvsXmlFile = "kvs"+ str(kvsId) +".xml"
kvsIniFile = "kvstest"+ str(kvsId) +".ini"
...
class KVSTest():
    ...
    def initSolvers(self):
        ...
        self.multiMDCellService = MultiMDCellService(self.mdSolverInterface, self.macroscopicSolverInterface,
                                                     ...
                                                     self.mamicoConfig, kvsXmlFile, self.multiMDService)
...
class LBSolver():
    ...
    def setup_scenario(self):
        ...
        def obstacle(x, y, z):
            return (x > 0.475*self.cpm) & (x < 0.525*self.cpm) & (y > 0.175*self.cpm) & (y < 0.225*self.cpm)
